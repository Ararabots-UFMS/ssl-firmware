...
int main(void)
{
  ...
  uint32_t rx_int;
  char tx_buff[50];
  sprintf(tx_buff, "Waiting for first input\r\n");
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  while (HAL_UART_Receive(&huart1, rx_buff, strlen(rx_buff), 1000) != HAL_OK)
	  {
	  ...
	  sscanf(rx_buff, "%lu", &rx_int);
	  sprintf(tx_buff, "%lu\r\n", rx_int);
	  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, rx_int);
