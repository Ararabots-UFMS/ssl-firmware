...
float map(float in_value) {
	if (in_value == 0)
		return 0;
	else if (in_value > 200)
		return 0;
	else if (in_value > motor_max)
		return pwm_max;
	return (in_value - motor_min) * (pwm_max - pwm_min)
			/ (motor_max - motor_min) + pwm_min;
}
...
uint8_t is_safe(float current_value, int8_t current_direction,
		uint8_t motor_idex) {
	if ((last_values[motor_idex] > 60.0 && current_value > 60.0)
			&& (last_direction[motor_idex] != current_direction)) {
		return 0;
	}
	return 1;
}
...
void write_speed_to_motors(TIM_TypeDef *TIMER, float *inverse_kinematics) {
	...
	/*
	 printf("[%d, %d, %d, %d]\r\n", last_direction[0] * last_values[0],
	 last_direction[1] * last_values[1],
	 last_direction[2] * last_values[2],
	 last_direction[3] * last_values[3]);
	 */
