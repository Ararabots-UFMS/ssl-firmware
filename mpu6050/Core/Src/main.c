...
int main(void)
{
  ...
  //uint32_t previousTime, elapsedTime;
  //uint16_t delay = 100;
  //float yaw = 0;
  char tmp[40];
  sprintf(tmp, "initializing\r\n");
  ...
  while (1)
  {
	...
	//yaw = yaw + MPU6050.Gz*10;
	sprintf(tmp, "acc: %f/%f/%f\r\n", MPU6050.Ax, MPU6050.Ay, MPU6050.Az);
	HAL_UART_Transmit(&huart1, (uint8_t *)tmp, strlen(tmp), 10);
	sprintf(tmp, "gyro: %f/%f/%f\r\n",MPU6050.Gx, MPU6050.Gy, MPU6050.Gz);
	HAL_UART_Transmit(&huart1, (uint8_t *)tmp, strlen(tmp), 10);
	HAL_Delay(100);
  }
  /* USER CODE END 3 */
...
void calculate_IMU_error(MPU6050_t MPU6050) {
  ...
  while (c < error_iterations) {
	...
	AccErrorZ = AccErrorZ + MPU6050.Az;
