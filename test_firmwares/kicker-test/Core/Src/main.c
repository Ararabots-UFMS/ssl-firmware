...
int main(void)
{
  ...
  while (1)
  {
	  int ir = HAL_GPIO_ReadPin(INFRARED_GPIO_Port, INFRARED_Pin); // pino do infrared sensor
	  HAL_GPIO_WritePin(KICKER_GPIO_Port, KICKER_Pin, GPIO_PIN_RESET);

	  if (ir == 0) {
	  		HAL_GPIO_WritePin(KICKER_GPIO_Port, KICKER_Pin, GPIO_PIN_SET); // kicker acionado
	  		HAL_Delay(5);
	  		HAL_GPIO_WritePin(KICKER_GPIO_Port, KICKER_Pin, GPIO_PIN_RESET);
